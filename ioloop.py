...
"""
A specialized IO loop on top of asyncore's adding support for epoll()
and kqueue() on Linux and OSX/BSD.
This is not supposed to be used directly.
Use FTPServer.server_forever() instead.
"""
...
def install(poller):
    ...
    if ioloop._instance is not None:
        raise ValueError("iopoller must be stopped first")
    ioloop = poller
